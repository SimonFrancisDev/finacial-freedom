...
export const Orbits = () => {
  ...
  const starConfig = getStarConfig(40)
    ...
    const getPlanetBadgeValue = useCallback((position) => {
      ...
      // Primary source: Receipt data (most accurate)
      if (receiptsSupported && position.viewerReceiptBreakdown) {
        return Number(position.viewerReceiptBreakdown.totalLiquid || 0)
      }

      // Fallback: Old rule view (only if receipts not available)
      return Number(position?.positionInfo?.exactToOwner || 0)
    }, [receiptsSupported])


    // === FINAL FIXED Tooltip - Now correctly shows $20 locked for owner on Line 3 ===
  const renderPositionTooltip = (position) => {
    ...
    // === NEW: Correct escrow calculation for owner (especially Line 3) ===
    const ownerEscrow = receiptsSupported && position.payoutReceipts?.length > 0
      ? position.payoutReceipts.reduce((sum, r) => {
          return sum + (Number(r.escrowLocked || 0) * (r.receiver === position.orbitOwner ? 1 : 0))
        }, 0)
        ...
        {receiptsSupported && position.payoutReceipts?.length > 0 ? (
          position.payoutReceipts.map((receipt, idx) => {
            ...
            return (
              <div className="small" key={idx}>
                {typeName}: {formatUsdtDisplay(receipt.grossAmount)} USDT → {fmtAddr(receipt.receiver)}
          ...
          <>
            ...
            {position.positionInfo?.exactToSpillover1 > 0 && (
              <div className="small">Spillover 1: {formatUsdtDisplay(position.positionInfo.exactToSpillover1)} USDT</div>
            )}
            {position.positionInfo?.exactToSpillover2 > 0 && (
              <div className="small">Spillover 2: {formatUsdtDisplay(position.positionInfo.exactToSpillover2)} USDT</div>
            )}
          </>
        )}

        <div className="mt-2">
          <strong>Escrow locked for orbit owner:</strong>{' '}
          {formatUsdtDisplay(ownerEscrow)} USDT
        </div>
                      ...
                      selectedPosition.payoutReceipts.map((receipt, idx) => {
                        ...
                        return (
                          <div className="commission-item" key={idx}>
                            <span>
                              {typeName} → {shortAddress(receipt.receiver)}
