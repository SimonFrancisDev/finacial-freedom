...
export const Orbits = () => {
  ...
  const normalizeRuleView = (ruleResult) => {
    ...
    return {
      ...
      hasStoredRuleData: Boolean(ruleResult.hasStoredRuleData ?? ruleResult[4] ?? false),
  ...
  const buildPositionInfoFromRuleView = (orbitType, position, level, ruleView) => {
...
const fetchStoredCycleForLevel = useCallback(async (level, cycleNumber) => {
    ...
    for (let pos = 1; pos <= config.positions; pos++) {
      ...
      const posInfo = buildPositionInfoFromRuleView(orbitType, pos, level, historicalRuleView)
              ...
              for (let pos = 1; pos <= config.positions; pos++) {
                positionTasks.push(async () => {
                  try {
                    ...
                    const posInfo = buildPositionInfoFromRuleView(orbitType, pos, level, ruleView)
                    ...
                    const posInfo = buildPositionInfoFromRuleView(orbitType, pos, level, null)
                    return {
                      number: pos,
                      occupantType: 'empty',
                      occupant: null,
      ...
      <Tooltip id={`tooltip-${position.number}`}>
        ...
        {position.positionInfo?.exactToOwner > 0 && (
          <div className="text-success mt-1">
            Owner: {position.positionInfo.exactToOwner} USDT
          </div>
        )}

        {position.positionInfo?.exactToSpillover1 > 0 && (
          <div className="text-warning mt-1">
            Spillover 1: {position.positionInfo.exactToSpillover1} USDT
          </div>
        )}

        {position.positionInfo?.exactToSpillover2 > 0 && (
          <div className="text-warning mt-1">
            Spillover 2: {position.positionInfo.exactToSpillover2} USDT
          </div>
        )}

        {position.positionInfo?.exactToEscrow > 0 && (
          <div className="text-info mt-1">
            Escrow: {position.positionInfo.exactToEscrow} USDT
          </div>
        )}

        {position.positionInfo?.exactToRecycle > 0 && (
          <div className="text-secondary mt-1">
            Recycle: {position.positionInfo.exactToRecycle} USDT
          </div>
        )}
      </Tooltip>
    )
                ...
                <>
                  ...
                  <div className="commission-breakdown">
                    ...
                    {selectedPosition.positionInfo?.exactToOwner > 0 && (
                      <div className="commission-item">
                        ...
                        <span className="commission-amount payout">
                          {selectedPosition.positionInfo.exactToOwner} USDT
                        </span>
                      </div>
                    )}
                    ...
                    {selectedPosition.positionInfo?.exactToSpillover1 > 0 && (
                      <div className="commission-item">
                        <span>To spillover 1</span>
                        <span className="commission-amount" style={{ color: '#ffc107' }}>
                          {selectedPosition.positionInfo.exactToSpillover1} USDT
                        </span>
                      </div>
                    )}
                    ...
                    {selectedPosition.positionInfo?.exactToSpillover2 > 0 && (
                      <div className="commission-item">
                        <span>To spillover 2</span>
                        <span className="commission-amount" style={{ color: '#ffc107' }}>
                          {selectedPosition.positionInfo.exactToSpillover2} USDT
                        </span>
                      </div>
                    )}
                    ...
                    {selectedPosition.positionInfo?.exactToEscrow > 0 && (
                      <div className="commission-item">
                        ...
                        <span className="commission-amount escrow">
                          {selectedPosition.positionInfo.exactToEscrow} USDT
                        </span>
                      </div>
                    )}
                    ...
                    {selectedPosition.positionInfo?.exactToRecycle > 0 && (
                      <div className="commission-item">
                        <span>To recycle</span>
                        <span className="commission-amount" style={{ color: '#6c757d' }}>
                          {selectedPosition.positionInfo.exactToRecycle} USDT
                        </span>
                      </div>
                    )}
                        ...
                        <div className="galaxy-inner">
                          {(() => {
                            ...
                            const createEmptyPosition = (posNumber, lineNum) => ({
                              ...
                              amount: '0',
                              timestamp: 0,
                              positionInfo: buildPositionInfoFromRuleView(orbitType, posNumber, level, null),
                              line: lineNum,
                              spillsTo: null,
                                        ...
                                        >
                                          ...
                                          >
                                            <div className="planet-content">
                                              ...
                                              {pos.positionInfo.exactToOwner > 0 && pos.occupantType !== 'mine' && (
                                                <span className="planet-earn-badge">
                                                  {pos.positionInfo.exactToOwner}
                                                </span>
                                              )}
                                            </div>
                                          </div>
...
//   /* ===== MOBILE LAYOUT OPTIMIZATION - FULL WIDTH ORBIT CARD ===== */
//   @media (max-width: 991px) {
//     .row > .col-lg-8,
//     .row > .col-lg-4 {
//       width: 100% !important;
